# Claude Code Project Instructions — filedescriptor-kotlin

## Project Overview

This is **filedescriptor-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`filedescriptor`](https://crates.io/crates/filedescriptor).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/filedescriptor/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/filedescriptor>
